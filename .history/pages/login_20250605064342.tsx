...
const LoginPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Use the same API route pattern as register
      ...
      if (response.ok) {
        ...
        // Get userId
        const userId = result.user.id || result.user._id || result.user.user_id || result.user.ID;
  ...
  const handleOtpVerification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        // Ensure user data has consistent properties before storing
        const userData = {
          ...tempUser,
