...
const useWebSocketWithGeofence = (userId?: string, user?: User | null) => {
  ...
  // Check geofence violations using detector
  const checkGeofenceViolations = useCallback((vehicle: Vehicle, latestData: VehicleData) => {
    ...
    if (!latestData?.latitude || !latestData?.longitude || !vehicle.geofence_id) {
      console.log('❌ No GPS data or geofence assigned');
      return;
    }
    ...
    if (result.triggeredAlert && result.events.length > 0) {
      console.log('🚨 Violations detected:', result.events);
  ...
  }, [userId, vehicles, checkGeofenceViolations, addOrUpdateGeofence, removeGeofenceById]);
