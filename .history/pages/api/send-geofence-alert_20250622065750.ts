...
Email ini dikirim otomatis oleh sistem GPS Tracker.
    ...
    try {
      // Approach 1: Try direct require
      const nodemailer = require('nodemailer');
      console.log('Using require approach');
      ...
      console.log('Require approach failed, trying dynamic import...');
      
      try {
        // Approach 2: Dynamic import with different access pattern
        const nodemailerModule = await import('nodemailer');
        console.log('Nodemailer module keys:', Object.keys(nodemailerModule));
        
        // Try different property access
        const createTransporter = nodemailerModule.createTransporter || 
                                 nodemailerModule.default?.createTransporter ||
                                 (nodemailerModule as any).createTransporter;
        
        if (!createTransporter) {
          throw new Error('createTransporter function not found in nodemailer module');
        }
        ...
        console.error('All import approaches failed:', importError);
