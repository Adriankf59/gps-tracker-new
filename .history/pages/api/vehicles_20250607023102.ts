...
async function handleGetVehicles(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 🔥 FIXED: Construct URL without problematic fields
    ...
    // 🔥 REMOVED: Sorting by created_at (causes permission error)
    ...
    params.append('fields', allowedFields.join(','));
    ...
    // 🔥 ADD MOCK TIMESTAMPS FOR COMPATIBILITY
    const vehiclesWithTimestamps = vehicles.map((vehicle: any) => ({
      ...vehicle,
    ...
    return res.status(200).json({
      ...
      count: vehiclesWithTimestamps.length,
      user_id: user_id
    });
...
async function handleCreateVehicle(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 🔥 REMOVED: Don't add timestamps to avoid permission issues
