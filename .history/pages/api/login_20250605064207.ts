...
export default async function handler(
  ...
  try {
    ...
    // Find user with matching email or nickname (for flexibility)
    const user = users.find((u: any) => 
      u.email === sanitizedEmail || 
      u.nickname === sanitizedEmail.split('@')[0] // Allow login with username
    );
    ...
    // Prepare user data with CONSISTENT field mapping
    const userData = {
      // Primary ID fields - ensure both exist for compatibility
      id: user.users_id, // Use users_id as primary ID
      user_id: user.users_id, // Also provide user_id for compatibility
      users_id: user.users_id, // Keep original field name
      
      // User information from database structure
      name: user.name, // Full name
      full_name: user.name, // Alias for compatibility
      nickname: user.nickname, // Username
      username: user.nickname, // Alias for compatibility
      email: user.email,
    ...
    // Log successful login with proper user_id
    console.log('✅ User login successful:', {
      user_id: userData.users_id, // Use the actual field from database
    ...
    if (redis) {
      try {
        await redis.setex(
          `login_session:${userData.users_id}`, // Use users_id for consistency
          24 * 60 * 60, // 24 hours
          JSON.stringify({
