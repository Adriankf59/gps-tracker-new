...
const isVehicleInsideGeofence = (vehiclePos: [number, number], geofence: ProjectGeofence): boolean => {
  console.log('🎯 Checking if vehicle is inside geofence:', {
    vehiclePos,
  ...
  if (!geofence || !geofence.definition) {
    console.log('❌ Invalid geofence or definition');
    return false;
  }
  ...
  try {
    if (geofence.type === 'circle') {
      ...
      if (!center || !radius) {
        console.log('❌ Circle geofence missing center or radius');
        return false;
      }
      const isInside = isPointInCircle(vehiclePos, center, radius);
      console.log(`📍 Circle check: ${isInside ? 'INSIDE' : 'OUTSIDE'} (radius: ${radius}m)`);
      return isInside;
    }
    ...
    if (geofence.type === 'polygon') {
      ...
      if (!coordinates || !coordinates[0]) {
        console.log('❌ Polygon geofence missing coordinates');
        return false;
      }
      const isInside = isPointInPolygon(vehiclePos, coordinates[0]);
...
// WebSocket Hook with Geofence Detection - UPDATED TO ACCEPT USER
const useWebSocketWithGeofence = (userId?: string, user?: User | null) => {
  ...
  const sendEmailNotification = useCallback(async (
    ...
    try {
      ...
      // Email failure shouldn't block the main alert flow
    }
  }, []);

  // Check geofence violations
  const checkGeofenceViolations = useCallback((vehicle: Vehicle, latestData: VehicleData) => {
    console.log('🔍 Checking geofence for vehicle:', vehicle.name, vehicle.vehicle_id);
    ...
    if (!assignedGeofence) {
      console.log('❌ No active geofence assigned to vehicle');
      return;
    }
    
    if (!assignedGeofence.definition) {
      console.log('❌ Geofence has no definition');
      return;
    }

    console.log('✅ Found assigned geofence:', assignedGeofence.name, assignedGeofence.rule_type);
    ...
    console.log('Geofence check:', {
      wasInside,
      isCurrentlyInside,
      ruleType: assignedGeofence.rule_type
    });
    ...
    if (violationDetected && alertType) {
      console.log('🚨 VIOLATION DETECTED!', alertMessage);
      ...
      if ('Notification' in window && Notification.permission === 'granted') {
        ...
        new Notification('Pelanggaran Geofence!', {
          ...
          tag: `geofence-${vehicle.vehicle_id}-${alertType}`,
      ...
      console.log('✅ Geofence violation handling complete');
    } else {
      console.log('ℹ️ No violation detected');
    }
  }, [geofences, vehiclePositionHistory, saveAlertToAPI, sendEmailNotification, user]);
  ...
  const handleRealtimeUpdate = useCallback((message: any) => {
    ...
    if (data.vehicle_id && data.license_plate) {
      ...
      console.log('📍 GPS data update:', data);
      setVehicleData(prev => {
      ...
      console.log('🔎 Looking for vehicle with GPS ID:', data.gps_id);
      console.log('Available vehicles:', vehicles);
      
      const vehicle = vehicles.find(v => v.gps_id === data.gps_id);
      if (vehicle) {
        console.log('✅ Found vehicle:', vehicle.name, 'with geofence_id:', vehicle.geofence_id);
        if (vehicle.geofence_id) {
          console.log('🚀 Triggering geofence check for vehicle:', vehicle.name);
          checkGeofenceViolations(vehicle, data);
        } else {
          console.log('⚠️ Vehicle has no geofence assigned');
        }
      } else {
        console.log('❌ No vehicle found with GPS ID:', data.gps_id);
      }
...
const DashboardPage = () => {
  ...
  const { 
    ...
    setActiveGeofenceAlert,
    loadInitialData
  } = useWebSocketWithGeofence(user?.id || user?.user_id, user); // Pass user here
