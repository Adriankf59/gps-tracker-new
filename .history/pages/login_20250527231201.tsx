...
const LoginPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        ...
        // Show OTP step
        setShowOtpStep(true);
        toast.success('Login berhasil! Gunakan OTP: 123456');
  ...
  const handleOtpVerification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Accept any 6-digit OTP for demo, or use "123456" as valid OTP
      if (otp.length === 6 && (otp === '123456' || /^\d{6}$/.test(otp))) {
        // Get user data from temporary storage
        const tempUser = sessionStorage.getItem('tempUser');
        console.log('Temp user data:', tempUser);
        
        if (tempUser) {
          // Store user data permanently in sessionStorage (same as dashboard)
  ...
  const handleResendOtp = () => {
    ...
    toast.success('OTP telah dikirim ulang ke email Anda. Demo: gunakan 123456');
  };
              ...
              <form onSubmit={handleOtpVerification} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-blue-600 text-center bg-blue-50 p-2 rounded">
                    Demo: Gunakan OTP <strong>123456</strong> untuk login
                  </p>
                </div>
