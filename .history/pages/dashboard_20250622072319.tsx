...
const useWebSocketWithGeofence = (userId?: string, user?: User | null) => {
  ...
  const checkGeofenceViolations = useCallback((vehicle: Vehicle, latestData: VehicleData) => {
    ...
    console.log('🗺️ Available geofences:', geofences);
    ...
    if (!assignedGeofence) {
      ...
      console.log('Available geofence IDs:', geofences.map(g => g.geofence_id));
      console.groupEnd();
      return;
    }
    ...
    console.groupEnd();
  }, [detectVehicleEvents, handleGeofenceEvents, geofences]);
  ...
  const handleRealtimeUpdate = useCallback((message: any) => {
    ...
    if (data.vehicle_id && data.license_plate) {
      ...
      // Debug: Show all vehicles and their GPS IDs
      console.log('🔎 Searching for vehicle with GPS ID:', data.gps_id);
      console.log('Number of vehicles available:', vehicles.length);
      
      if (vehicles.length === 0) {
        console.log('⚠️ No vehicles loaded yet! Skipping geofence check.');
        console.groupEnd();
        return;
      }
      ...
      let vehicle = vehicles.find(v => v.gps_id === data.gps_id);
      ...
      if (!vehicle && data.vehicle_id) {
        ...
        vehicle = vehicles.find(v => v.vehicle_id === data.vehicle_id);
      }
    ...
    console.groupEnd();
  }, [userId, vehicles, checkGeofenceViolations, addOrUpdateGeofence, removeGeofenceById, geofences]);
...
const DashboardPage = () => {
  ...
  }, [vehicles, geofences, vehicleData, user, isConnected, checkGeofenceViolations]);
  ...
  return (
    ...
    >
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="fixed bottom-20 right-4 bg-black/90 text-white p-4 rounded-lg text-xs max-w-md max-h-96 overflow-auto z-[9998]">
          <h3 className="font-bold mb-2">🐛 Debug Info</h3>
          <div className="space-y-1">
            <p>User: {user?.email || 'No user'}</p>
            <p>WebSocket: {isConnected ? '✅ Connected' : '❌ Disconnected'}</p>
            <p>Vehicles: {vehicles.length}</p>
            <p>Active Geofences: {geofences.filter(g => g.status === 'active').length}</p>
            <p>Vehicle Data Points: {vehicleData.length}</p>
            <details className="mt-2">
              <summary className="cursor-pointer">Vehicles with Geofences</summary>
              <div className="ml-2 mt-1">
                {vehicles.map(v => (
                  <div key={v.vehicle_id} className="text-xs">
                    {v.name}: {v.geofence_id ? `Geofence ${v.geofence_id}` : 'No geofence'}
                  </div>
                ))}
              </div>
            </details>
            <details className="mt-2">
              <summary className="cursor-pointer">Active Geofences</summary>
              <div className="ml-2 mt-1">
                {geofences.filter(g => g.status === 'active').map(g => (
                  <div key={g.geofence_id} className="text-xs">
                    ID: {g.geofence_id} - {g.name} ({g.rule_type})
                  </div>
                ))}
              </div>
            </details>
