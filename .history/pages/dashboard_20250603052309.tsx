...
const DashboardPage = () => {
  ...
  const initializeSystemStatus = useCallback(async () => {
    try {
      ...
      // Simulate real-time data dengan cara yang predictable
      const deviceVariation = Math.floor((currentTime.getMinutes() % 10) / 2); // 0-4 variation based on time
      const trackingVariation = Math.floor((currentTime.getSeconds() % 8) / 2); // 0-3 variation based on time
      ...
      const intervalId = setInterval(() => {
        ...
        setSystemStatus(prev => ({
          ...prev,
          lastUpdate: now,
          // Update values dengan predictable pattern
          connectedDevices: baseDevices + Math.floor((now.getMinutes() % 10) / 2),
          activeTracking: baseTracking + Math.floor((now.getSeconds() % 8) / 2)
        }));
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-slate-100">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-600">Preparing dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <SidebarProvider>
      <div className={`flex min-h-screen w-full bg-gradient-to-br from-slate-50 to-slate-100 transition-all duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
        {/* Enhanced Sidebar */}
        <AppSidebar 
