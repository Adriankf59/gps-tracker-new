...
const LoginPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Debug API headers:', Object.fromEntries(debugResponse.headers.entries()));
      ...
      if (debugResponse.ok) {
        ...
        console.log('Debug API error response:', debugText);
      }
      
      // Now try the actual login API
  ...
  const handleOtpVerification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (otp.length === 6) {
        // Verify login again to get user data
        const response = await fetch('/api/auth/login', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
