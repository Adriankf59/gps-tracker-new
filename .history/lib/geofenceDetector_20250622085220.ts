...
export class GeofenceDetector {
  ...
  public updateVehicleData(
    ...
    timestamp: Date,
    forceInitialCheck: boolean = false
  ): DetectionResult {
    ...
    const isFirstCheck = !this.vehicleStates.has(vehicleId);
    ...
    for (const geofence of this.geofences.values()) {
      ...
      // Jika ini adalah pengecekan pertama atau forceInitialCheck, dan kendaraan berada dalam zona terlarang
      if ((isFirstCheck || forceInitialCheck) && isCurrentlyInside && geofence.rule_type === 'FORBIDDEN') {
        eventType = 'violation_enter';
        result.triggeredAlert = true;
        console.log(`🚨 Initial check: Vehicle ${vehicleId} is ALREADY INSIDE forbidden zone ${geofence.name}!`);
      } 
      // Jika ini adalah pengecekan pertama atau forceInitialCheck, dan kendaraan berada di luar zona STAY_IN
      else if ((isFirstCheck || forceInitialCheck) && !isCurrentlyInside && geofence.rule_type === 'STAY_IN') {
        eventType = 'violation_exit';
        result.triggeredAlert = true;
        console.log(`🚨 Initial check: Vehicle ${vehicleId} is OUTSIDE stay-in zone ${geofence.name}!`);
      }
      // Normal transition checks
      else if (isCurrentlyInside && !wasInside) { // Kendaraan baru saja MASUK
...
export const useProjectGeofenceDetection = () => {
  const detectVehicleEvents = React.useCallback((
    ...
    return geofenceDetectorInstance.updateVehicleData(vehicleId, currentPosition, timestamp, forceInitialCheck);
