...
export function GeofenceManager() {
  ...
  const handleDeleteGeofence = async (geofenceId: number) => {
    if (!confirm("Apakah Anda yakin ingin menghapus geofence ini?")) return;

    setLoading(true);
    try {
      // Unassign vehicles first
      const assignedVehicles = vehicles.filter(v => v.geofence_id === geofenceId.toString());
      await Promise.all(assignedVehicles.map(v => updateVehicleGeofence(v.vehicle_id, null)));
                      ...
                      <Checkbox 
                        ...
                        onCheckedChange={() => {
                          if (isAssignedElsewhere) {
                            toast.error(`${vehicle.name} sudah di-assign ke geofence lain`);
                            return;
                          }
