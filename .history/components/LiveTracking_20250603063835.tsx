...
// Commented out imports that might not exist - you can uncomment if available
// import {
...
export function LiveTracking() {
  ...
  // Uncomment if geofence detection is available
  // const {
  //   detectVehicleEvents,
  //   addOrUpdateGeofence,
  ...
  const {
    ...
    {
      ...swrConfig,
      refreshInterval: 60000, // Less frequent for geofences
      onError: (error) => {
        console.error('Geofences loading error:', error);
        // Don't show toast for geofences as they're optional
      }
    }
  );
  ...
  const {
    ...
    {
      ...swrConfig,
      refreshInterval: 10000, // More frequent for real-time data
      onError: (error) => {
  ...
  const processedVehicles = useMemo((): VehicleWithTracking[] => {
    ...
    // Create efficient lookup map for latest data
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Handle tracked vehicle from session
  ...
  // Geofence detection effects (commented out - uncomment if available)
  /*
  useEffect(() => {
    if (vehicles.length > 0) {
      const vehiclesForDetection = vehicles
        .filter(v => v.gps_id !== null)
        .map(v => ({ ...v, gps_id: v.gps_id as string }));
      setVehiclesDetailForDetection(vehiclesForDetection);
    }
            ...
            <CardContent className="p-0">
              ...
              >
                <MapComponent
                  ...
                  onMapClick={() => {}} // Keep selected vehicle
