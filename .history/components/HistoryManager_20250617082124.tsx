...
export function HistoryManager() {
  ...
  const processedVehicleData = useMemo((): VehicleData[] => {
    if (!selectedVehicle || showAllVehicles) return [];
    
    const vehicle = vehicles.find(v => v.vehicle_id === selectedVehicle);
    if (!vehicle?.gps_id) return [];
    
    // Filter data for selected vehicle only
    const vehicleSpecificData = filteredVehicleData.filter(data => data.gps_id === vehicle.gps_id);
    
    console.log('Processing vehicle data:', vehicleSpecificData.length, 'raw records for', vehicle.name);
    
    // Filter valid data with coordinate and timestamp validation
    const validData = vehicleSpecificData.filter((item: VehicleData) => {
    ...
    if (sortedData.length === 0 && vehicleSpecificData.length > 0) {
    ...
    return sortedData;
  }, [filteredVehicleData, selectedVehicle, showAllVehicles, vehicles]);
  ...
  const routeMarkers = useMemo((): ProcessedVehicleForMap[] => {
    if (optimizedData.length === 0 || !selectedVehicle) return [];
    ...
    return markers;
  }, [optimizedData, selectedVehicle, vehicles]);
  ...
  const stats = useMemo(() => {
    const dataToAnalyze = processedVehicleData;
    ...
    return {
      ...
      vehicleCount: showAllVehicles ? vehicles.length : 1
    };
  }, [processedVehicleData, calculateDistance, showAllVehicles, vehicles.length]);
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 bg-slate-50 min-h-screen">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              {!showAllVehicles && selectedVehicle && routePolyline.length > 0 && (
          ...
          <div className="h-[600px] rounded-b-xl overflow-hidden m-4">
            {vehicles.length === 0 ? (
              ...
              <MapComponent
                ...
                onMapClick={() => {}}
                routePolyline={!showAllVehicles && routePolyline.length > 0 ? routePolyline : undefined}
      ...
      {selectedVehicle && !showAllVehicles && processedVehicleData.length > 0 && (
        <Card className="shadow-md">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
