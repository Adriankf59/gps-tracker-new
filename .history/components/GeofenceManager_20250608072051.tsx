...
export function GeofenceManager() {
  ...
  // 🆕 Dedicated save function with enhanced error handling
  const saveGeofenceToAPI = useCallback(async (payload: any) => {
    try {
      console.log('💾 Saving geofence to API...', payload);
      ...
      console.log('✅ Save operation response:', result);
      
      // 🆕 Enhanced response parsing - handle different response formats
      let savedGeofenceId = null;
      
      if (result.geofence_id) {
        savedGeofenceId = result.geofence_id;
      } else if (result.data && result.data.geofence_id) {
        savedGeofenceId = result.data.geofence_id;
      } else if (result.id) {
        savedGeofenceId = result.id;
      } else if (result.data && result.data.id) {
        savedGeofenceId = result.data.id;
        ...
        console.log('🔍 Save response doesn\'t contain ID, but appears successful');
        return { success: true, data: result };
      }
      
      if (savedGeofenceId) {
        console.log('✅ Extracted geofence ID:', savedGeofenceId);
        return { success: true, geofence_id: savedGeofenceId, data: result };
      } else {
        console.log('⚠️ No ID in response, assuming save was successful');
        return { success: true, data: result };
      }
    } catch (error: any) {
