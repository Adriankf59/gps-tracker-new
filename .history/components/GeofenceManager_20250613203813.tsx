...
const getGeofenceCenter = (geofence) => {
...
export function GeofenceManager() {
  ...
  const fetchGeofences = useCallback(async (userId) => {
    try {
      const result = await fetchData(`${GEOFENCE_API}?filter[user_id][_eq]=${userId}&limit=-1&sort=-date_created`);
      if (!result) return [];
      
      const parsed = ensureArray(result.data || result).map(gf => ({
        ...gf,
  ...
  const fetchVehicles = useCallback(async (userId) => {
    try {
      ...
      const fetched = ensureArray(result.data || result);
      setVehicles(fetched);
      return fetched;
    } catch (error) {
  ...
  const handleSaveGeofence = useCallback(async () => {
    ...
    try {
      ...
      if (typeof layer.getRadius === 'function') {
        ...
        const coords = (Array.isArray(latlngs[0]) ? latlngs[0] : latlngs).map(ll => [ll.lng, ll.lat]);
  ...
  const handleDeleteGeofence = useCallback(async (geofenceId) => {
  ...
  const handleAssignVehicles = (geofence) => {
  ...
  const getAssignedCount = useCallback((geofenceId) => {
