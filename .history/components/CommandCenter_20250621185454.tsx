...
interface Vehicle {
  ...
  relayStatus?: string;
  relayCommandStatus?: string; // New field for notifications only
  licensePlate?: string;
  make?: string;
  model?: string;
  year?: number;
...
interface CommandHistory {
  ...
  relay_command_status?: "sent" | "success" | "failed"; // New field for notifications
  issuedBy: string;
  timestamp: string;
...
export function CommandCenter() {
  ...
  const handleSendCommand = async () => {
    ...
    try {
      ...
      // Record command in history with "sent" status for notification
      const commandData = {
        vehicle_id: selectedVehicle,
        command_type: selectedCommand,
        issued_by: userId,
        status: 'pending',
        relay_command_status: 'sent', // For notification tracking
      ...
      // Update vehicle relay status (keep existing logic that works)
      const newRelayStatus = selectedCommand === 'RELAY_ON' ? 'ON' : 'OFF';
      
      console.log(`📤 Updating vehicle ${selectedVehicle} relay to ${newRelayStatus}`);
      ...
      const vehicleUpdateResponse = await fetch(`${API_BASE_URL}/items/vehicle/${selectedVehicle}`, {
        ...
        body: JSON.stringify({
          ...
          relay_command_status: 'sent' // For notification display only
        }),
      });
      ...
      // Mark command as completed and update notification status
      const historyResult = await historyResponse.json();
      if (historyResult?.data?.id) {
        await fetch(`${API_BASE_URL}/items/commands/${historyResult.data.id}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            status: 'completed',
            relay_command_status: 'success', // Update notification status
            date_executed: new Date().toISOString()
          }),
        });
        
        // Update vehicle notification status to success
        await fetch(`${API_BASE_URL}/items/vehicle/${selectedVehicle}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      toast.info(
        ...
        {
          description: `${formatCommandName(selectedCommand)} command has been sent`,
          duration: 3000,
        }
      );
      ...
      setTimeout(() => {
        toast.success(
          `✅ Command executed successfully on ${selectedVehicleObj?.name || 'vehicle'}`,
          {
            description: `${formatCommandName(selectedCommand)} completed`,
            duration: 4000,
          }
        );
      }, 1500);
              ...
              <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-xs sm:text-sm text-yellow-700">
                    ...
                    <p>Commands are executed immediately. Ensure vehicle safety first.</p>
                  </div>
                </div>
              </div>
            </CardContent>
