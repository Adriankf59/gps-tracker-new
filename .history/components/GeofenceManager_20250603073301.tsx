...
interface UIState {
  ...
  connectionError: boolean; // 🆕 Track connection/CORS errors
  errorDetails: string | null; // 🆕 Store error details for troubleshooting
}
...
export function GeofenceManager() {
  ...
  // 🔧 Enhanced API functions with CORS-compatible headers
  const fetchWithErrorHandling = useCallback(async (url: string, options?: RequestInit) => {
    try {
      ...
      // 🆕 CORS-safe headers - avoid Cache-Control header that causes CORS issues
      ...
      if (error.message?.includes('CORS') || error.message?.includes('Failed to fetch')) {
        ...
        console.error('🔧 Consider checking server CORS configuration or network connectivity');
      }
  ...
  // 🆕 Enhanced fetchGeofences with CORS-safe data freshness
  const fetchGeofences = useCallback(async (userId: string, options: { silent?: boolean; forceRefresh?: boolean } = {}) => {
    try {
      ...
      // 🆕 Use URL parameter instead of header for cache busting (CORS-safe)
      const freshParam = options.forceRefresh ? `&_t=${Date.now()}` : '';
      const apiUrl = `${GEOFENCE_API_ENDPOINT}?filter[user_id][_eq]=${userId}&limit=-1&sort=-date_created${freshParam}`;
      ...
      let errorMessage = "Failed to load geofences";
      let isConnectionError = false;
      
      if (error.message?.includes('CORS')) {
        errorMessage = "Server access blocked (CORS). Please check server configuration.";
        isConnectionError = true;
      } else if (error.message?.includes('Failed to fetch')) {
        errorMessage = "Network error. Please check your connection and server status.";
        isConnectionError = true;
      } else if (error.message?.includes('404')) {
        errorMessage = "Geofence service not found. Please check API endpoint.";
        isConnectionError = true;
  ...
  const fetchVehicles = useCallback(async (userId: string, options: { silent?: boolean } = {}) => {
    try {
      ...
      const apiUrl = `${VEHICLE_API_ENDPOINT}?filter[user_id][_eq]=${userId}&limit=-1`;
      console.log('📡 Vehicle API Request URL:', apiUrl);
      ...
      // 🆕 Better error messages for vehicles endpoint
      let errorMessage = "Failed to load vehicles";
      if (error.message?.includes('CORS')) {
        errorMessage = "Vehicle service access blocked (CORS). Please check server configuration.";
      } else if (error.message?.includes('Failed to fetch')) {
        errorMessage = "Network error loading vehicles. Please check your connection.";
      } else if (error.message?.includes('404')) {
        errorMessage = "Vehicle service not found. Please check API endpoint.";
      }
  ...
  // 🆕 Function to refresh all data with error recovery
  const refreshAllData = useCallback(async (userId: string, options: { silent?: boolean; forceRefresh?: boolean } = {}) => {
    ...
    try {
      ...
      if (fetchedGeofences && fetchedVehicles) {
        setUIState(prev => ({ 
          ...prev, 
          connectionError: false, 
          errorDetails: null 
      ...
      // 🆕 Set error state for connection issues
      const isConnectionError = error.message?.includes('CORS') || error.message?.includes('Failed to fetch');
      setUIState(prev => ({
        ...prev,
        connectionError: isConnectionError,
        errorDetails: error.message
  ...
  // 🆕 Manual refresh handler with error recovery
  const handleManualRefresh = useCallback(async () => {
    ...
    try {
      ...
      // 🆕 Update error state if refresh fails
      const isConnectionError = error.message?.includes('CORS') || error.message?.includes('Failed to fetch');
      setUIState(prev => ({
        ...prev,
        connectionError: isConnectionError,
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mb-6 pb-4 border-b border-slate-200/60">
        <div className="flex items-center gap-3 mb-4 sm:mb-0">
          ...
          <div>
            ...
            {/* 🆕 Data status indicator with network status */}
            {uiState.dataLoadTimestamp > 0 && (
              <div className="text-xs text-slate-400 mt-1 flex items-center gap-2">
                <span>Last updated: {new Date(uiState.dataLoadTimestamp).toLocaleTimeString()}</span>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-red-700 text-sm mb-3">
                      Unable to connect to the server. This is typically caused by CORS (Cross-Origin Resource Sharing) restrictions.
                    </p>
                    ...
                    <div className="bg-white/70 rounded-lg p-3 mb-3">
                      ...
                      <ol className="text-sm text-red-700 space-y-1 list-decimal list-inside">
                        <li>Check if the server is running at: <code className="bg-red-100 px-1 rounded text-xs">{API_BASE_URL}</code></li>
                    ...
                    <div className="flex gap-2">
                      <Button
                        onClick={() => {
                          setUIState(prev => ({ ...prev, connectionError: false, errorDetails: null }));
                          handleManualRefresh();
                        }}
                        size="sm"
                      ...
                      <Button
                        onClick={() => setUIState(prev => ({ ...prev, connectionError: false, errorDetails: null }))}
                        variant="outline"
                        size="sm"
