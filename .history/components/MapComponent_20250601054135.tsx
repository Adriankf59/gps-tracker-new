...
const MapComponent: React.FC<MapComponentProps> = ({
  ...
  const getVehicleIcon = (vehicle: ProcessedVehicle) => {
    ...
    // Special handling for route start/end markers - convert to string for safety
    const vehicleIdStr = String(vehicle.id);
    if (vehicleIdStr.includes('start-') || vehicleIdStr.includes('end-')) {
      const type = vehicleIdStr.includes('start-') ? 'start' : 'end';
    ...
    <div className={`relative ${className}`} style={{ height, minHeight, width: '100%' }}>
      ...
      >
        ...
        {vehicles.map(vehicle => {
          ...
          const vehicleIdStr = String(vehicle.id);
          const isRouteMarker = vehicleIdStr.includes('start-') || vehicleIdStr.includes('end-');
            ...
            >
              <Popup>
                <div style={{ minWidth: '200px', fontFamily: 'Arial, sans-serif' }}>
                  <div style={{ fontWeight: 'bold', fontSize: '14px', marginBottom: '8px' }}>
                    {isRouteMarker ? 
                      `${vehicle.name} (${vehicleIdStr.includes('start-') ? 'Start Point' : 'End Point'})` : 
                      vehicle.name
                    }
                  </div>
