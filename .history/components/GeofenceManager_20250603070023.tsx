...
import { Shield, Search, Plus, MapPin, Trash2, Circle as CircleIcon, Square, Save, X, Car, Loader2, AlertCircle } from "lucide-react";
...
export function GeofenceManager() {
  ...
  // 🔧 Optimized API functions with error handling and caching
  ...
  const fetchGeofences = useCallback(async (userId: string) => {
    try {
      ...
      const result = await fetchWithErrorHandling(
        `${GEOFENCE_API_ENDPOINT}?filter[user_id][_eq]=${userId}&limit=-1&sort=-date_created`
      );
      
      if (!result) return []; // Request was aborted
      
      console.log('📦 Raw geofences data:', result);
      
      const fetchedGeofences = ensureArray(result.data || result);
      console.log('📋 Fetched geofences array:', fetchedGeofences);
      
      // 🔧 Enhanced parsing with better error handling
      const parsedGeofences = fetchedGeofences.map((gf: any) => {
        console.log('🔧 Processing geofence:', gf.geofence_id, gf.name);
        
        if (typeof gf.definition === 'string') {
          try {
            const parsed = { ...gf, definition: JSON.parse(gf.definition) };
            console.log('✅ Parsed definition for', gf.name, ':', parsed.definition);
            return parsed;
          } catch (e) {
            console.error(`❌ Failed to parse definition for geofence ${gf.geofence_id}:`, e);
            // Keep geofence but with empty definition
      ...
      // 🔧 Less strict validation - show all geofences, mark invalid ones
      const processedGeofences = parsedGeofences.map(gf => ({
        ...gf,
        _isValid: validateGeofence(gf),
        _validationError: validateGeofence(gf) ? null : 'Invalid geometry data'
      }));
      
      setGeofences(processedGeofences);
      
      const validCount = processedGeofences.filter(gf => gf._isValid).length;
      const totalCount = processedGeofences.length;
      
      console.log(`✅ Loaded ${totalCount} geofences (${validCount} valid, ${totalCount - validCount} invalid)`);
      
      if (totalCount > 0) {
        toast.success(`Loaded ${totalCount} geofences`);
      } else {
        toast.info('No geofences found');
      }
      
      return processedGeofences;
    } catch (error: any) {
  ...
  const filteredGeofences = useMemo(() => {
    console.log('🔍 Filtering geofences:', geofences.length, 'total');
    
    if (!uiState.searchTerm.trim()) {
      console.log('📋 Showing all geofences');
      return geofences; // Show all, including invalid ones
    }
    ...
    console.log('🔍 Filtered to', filtered.length, 'geofences');
    return filtered;
  }, [geofences, uiState.searchTerm]);

  const validGeofences = useMemo(() => {
    const valid = geofences.filter(validateGeofence);
    console.log('✅ Valid geofences:', valid.length, 'out of', geofences.length);
    return valid;
  }, [geofences]);

  // 🔧 Optimized vehicle assignment functions
  ...
  const handleSaveGeofence = useCallback(async () => {
    ...
    try {
      ...
      if (result) {
        ...
        // Refresh geofences and set current
        const fetchedGeofencesList = await fetchGeofences(userId);
        const newGeo = result.data;
  ...
  }, [currentUser, newGeofence, drawnLayers, fetchWithErrorHandling, fetchGeofences]);
  ...
  }, [removeGeofenceFromVehicles, fetchWithErrorHandling, fetchGeofences, fetchVehicles, currentUser, currentGeofence]);
          ...
          <div className="flex-1 overflow-auto space-y-2 pr-1">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 mb-2">
              <p className="text-xs text-blue-700">
                📊 Total: {geofences.length} | Filtered: {filteredGeofences.length} | Valid: {validGeofences.length}
              </p>
            </div>
            ...
            {uiState.loading && geofences.length === 0 && !uiState.isCreating ? (
              ...
              <Card className="border-dashed border-slate-300 bg-gradient-to-br from-slate-50 to-white shadow-sm">
                <CardContent className="p-6 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-slate-700 mb-2">
                    {uiState.searchTerm ? "No results found" : geofences.length > 0 ? "No valid geofences" : "No geofences yet"}
                  </h3>
                  <p className="text-slate-500 mb-4 text-sm">
                    {uiState.searchTerm 
                      ? `No geofences match "${uiState.searchTerm}"` 
                      : geofences.length > 0
                        ? `Found ${geofences.length} geofences but none have valid geometry data`
                        : "Start by creating your first geofence"
                    }
                  </p>
                  {!uiState.searchTerm && geofences.length === 0 && (
                    <Button
                      onClick={handleStartCreating}
              ...
              filteredGeofences.map((geofence) => {
                const isValid = validateGeofence(geofence);
                    ...
                    className={`cursor-pointer transition-all duration-300 ease-out hover:shadow-xl border rounded-xl overflow-hidden transform hover:scale-[1.02] ${
                      isSelected
                        ...
                        : isValid
                          ...
                          : 'bg-red-50/80 backdrop-blur-sm border-red-200 hover:border-red-300 shadow-md'
                    }`}
                    onClick={() => {
                      if (isValid) {
                        setUIState(prev => ({ ...prev, isCreating: false }));
                        ...
                        toast.error("This geofence has invalid geometry data and cannot be displayed on the map");
                      }
                    }}
                  >
                    <CardContent className="p-3 sm:p-4">
                      ...
                      {/* Geometry Info */}
                      <div className="text-xs text-slate-500 mb-2 bg-slate-50 rounded px-2 py-1">
                        <div className="flex justify-between items-center">
                          <span>📅 {new Date(geofence.date_created).toLocaleDateString('en-US', {
                        ...
                        {!isValid && (
                          <div className="mt-1 text-red-600">
                            ⚠️ Invalid geometry data - cannot display on map
                          </div>
                        )}
                        {isValid && geofence.definition && (
                          <div className="mt-1 text-green-600">
                            ✅ Valid {geofence.type} geometry
                            {geofence.type === 'circle' && geofence.definition.radius && (
                              <span> - Radius: {Math.round(geofence.definition.radius)}m</span>
                            )}
                            {geofence.type === 'polygon' && geofence.definition.coordinates?.[0] && (
                              <span> - {geofence.definition.coordinates[0].length} points</span>
                            )}
                          </div>
                        )}
                      </div>
        ...
        <div className="lg:col-span-2 border border-slate-200 rounded-2xl overflow-hidden shadow-2xl bg-white/90 backdrop-blur-sm min-h-[300px] lg:min-h-0">
          ...
          <div className="bg-gray-50 border-b border-gray-200 px-4 py-2">
            <div className="flex justify-between items-center text-xs text-gray-600">
              <span>
                Map Mode: {uiState.isCreating ? '🎨 Creating' : '👀 Viewing'} | 
                Current: {currentGeofence ? currentGeofence.name : 'None'} |
                Draw: {drawMode}
              </span>
              <span>
                Valid Geofences: {validGeofences.length}
              </span>
            </div>
          </div>
