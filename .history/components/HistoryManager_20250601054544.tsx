...
export function HistoryManager() {
  ...
  // Initialize with today's date range and load MapComponent
  useEffect(() => {
    ...
    // Load MapComponent on client side
    if (typeof window !== 'undefined') {
      import('./MapComponent')
        ...
        .catch(error => {
          ...
          toast.error('Failed to load map component');
        });
    }
  }, []);
  ...
  // Enhanced processVehicleData with better logging
  ...
  useEffect(() => {
    if (selectedVehicle && dateRange?.from && dateRange?.to) {
      // Clear existing data when vehicle changes to prevent stale data
      setVehicleData([]);
      fetchVehicleData(selectedVehicle, dateRange.from, dateRange.to);
    } else {
      // Clear data if no vehicle selected
      setVehicleData([]);
    }
  ...
  // Enhanced vehicle selection handler with proper state reset
  const handleVehicleChange = (vehicleId: string) => {
    ...
    // If valid vehicle and date range, fetch data
            ...
            {loadingData ? (
              ...
              <div className="h-full flex items-center justify-center bg-slate-50 rounded-lg">
                <div className="text-center">
                  ...
                  <p className="text-slate-600">Map component failed to load</p>
                  <p className="text-sm text-slate-500">Please refresh the page</p>
                </div>
              </div>
            )}
          </div>
