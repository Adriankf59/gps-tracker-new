...
import { getAlerts } from "@/lib/alertService"; // Pastikan path ini benar
...
const useAlerts = () => {
  const { data, error, isLoading, mutate } = useSWR(
    ...
    async () => {
      // @ts-ignore
      const response = await getAlerts(); // Pastikan getAlerts adalah fungsi yang valid
  ...
  const processedAlerts = useMemo(() => {
    ...
    return data.map((alert: any): Alert => ({ // Berikan tipe Alert pada hasil map
...
export function Dashboard() {
  ...
  const vehiclesWithData: VehicleWithData[] = useMemo(() => {
    ...
    return vehicles.map((vehicle: Vehicle) => {
      ...
      return { ...vehicle, latestData, isOnline: online, location, status };
    });
  }, [vehicles, vehicleData]);

  // Process vehicles data for map component - PERBAIKAN DI SINI
  const processedVehiclesForMap = useMemo((): ProcessedVehicle[] => {
    // Langkah 1: Map vehicles to ProcessedVehicle or null
    const mappedVehiclesOrNull = vehiclesWithData
    ...
    // Langkah 2: Filter out null values menggunakan type guard
    return mappedVehiclesOrNull.filter(
      (item): item is ProcessedVehicle => item !== null
    );
  }, [vehiclesWithData]);
  ...
  const stats = useMemo(() => {
    ...
    const activeAlertsFromData = vehiclesWithData.filter(v => {
    ...
    return {
      totalVehicles,
      activeTracking,
      activeAlerts: Math.max(activeAlertsFromData, alerts.length),
      geofences: geofences.length
    };
  }, [vehiclesWithData, alerts, geofences.length]);
  ...
  const getAlertType = (alertType: string | null): string => {
    if (!alertType || typeof alertType !== 'string') return 'alert';
  ...
  if (isLoadingOverall && !vehiclesWithData.length) {
    return (
  ...
  if (hasErrorOverall && !vehiclesWithData.length) {
    return (
      <div className="flex items-center justify-center min-h-[calc(100vh-200px)]">
        <div className="text-center">
  ...
  const dashboardStats = [
    { title: "Total Vehicles", value: stats.totalVehicles.toString(), change: `${stats.activeTracking} online`, icon: Car, color: "blue" as const },
    { title: "Active Tracking", value: stats.activeTracking.toString(), change: `${stats.totalVehicles - stats.activeTracking} offline`, icon: MapPin, color: "green" as const },
    { title: "Active Alerts", value: stats.activeAlerts.toString(), change: "Fuel & Speed alerts", icon: AlertTriangle, color: "red" as const },
    { title: "Geofences", value: stats.geofences.toString(), change: "All active", icon: Shield, color: "purple" as const }
  ];
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-600">
            {userData ? `Welcome, ${userData.full_name || userData.username || userData.email}. Here's your fleet status.` : 'Loading your personal dashboard...'}
          </p>
        </div>
        <div
          onClick={handleRefresh}
          ...
          style={{ opacity: isLoadingOverall ? 0.7 : 1 }}
        >
          {isLoadingOverall ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />} Refresh
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 text-sm text-slate-600">
        ...
        {(vehicleDataLoading || alertsLoading) && <Loader2 className="w-3 h-3 animate-spin ml-2" />}
      </div>
        ...
        {dashboardStats.map((stat, index) => (
          <Card key={index} className="hover:shadow-lg transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <div className={`p-2 rounded-lg ${stat.color === 'blue' ? 'bg-blue-100' : stat.color === 'green' ? 'bg-green-100' : stat.color === 'red' ? 'bg-red-100' : 'bg-purple-100'}`}>
                <stat.icon className={`w-4 h-4 ${stat.color === 'blue' ? 'text-blue-600' : stat.color === 'green' ? 'text-green-600' : stat.color === 'red' ? 'text-red-600' : 'text-purple-600'}`} />
              </div>
            </CardHeader>
            <CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Clock className="w-5 h-5 text-blue-600" /> Recent Activity {alertsLoading && <Loader2 className="w-4 h-4 animate-spin ml-auto" />}
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {recentActivity.length > 0 ? (
                recentActivity.map((activity) => (
                  <div key={activity.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-slate-50">
                    <div className={`w-2 h-2 rounded-full mt-2 ${activity.type === 'alert' ? 'bg-red-500' : activity.type === 'geofence' ? 'bg-blue-500' : 'bg-green-500'}`} />
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-medium text-sm text-slate-800">{activity.vehicle}</span>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-green-600" /> Online Vehicles {vehicleDataLoading && <Loader2 className="w-4 h-4 animate-spin ml-auto" />}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {vehiclesWithData.filter(v => v.isOnline).length > 0 ? (
                vehiclesWithData.filter(v => v.isOnline).slice(0, 3).map((vehicle) => (
                  <div
                    ...
                    className={`p-4 border rounded-lg cursor-pointer transition-colors ${selectedVehicleId === vehicle.vehicle_id ? 'bg-blue-50 border-blue-200' : 'hover:bg-slate-50'}`}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant={vehicle.status === 'moving' ? 'default' : 'secondary'} className={`text-xs ${vehicle.status === 'moving' ? 'bg-green-100 text-green-700' : vehicle.status === 'parked' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700'}`}>
                          {vehicle.status}
                        </Badge>
                      </div>
                      <span className="text-sm font-medium text-blue-600">{vehicle.latestData?.speed ?? 0} km/h</span>
                    </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {selectedVehicleId && <Badge variant="outline" className="ml-auto">Vehicle Selected</Badge>}
