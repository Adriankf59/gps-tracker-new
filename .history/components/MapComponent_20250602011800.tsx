  ...
  useEffect(() => {
    ...
    vehicles.forEach((vehicle) => {
      if (
        ...
        const vehicleLatLng: LatLngExpression = [
          vehicle.position[0],
          vehicle.position[1],
        ];
        if (!bounds) {
...
// Konfigurasi ikon default Leaflet di sisi klien
...
const MapComponent: React.FC<MapComponentProps> = ({
  ...
  const initialCenter: [number, number] = useMemo(() => {
    if (
      centerCoordinates &&
      !isNaN(centerCoordinates[0]) &&
      !isNaN(centerCoordinates[1])
    ) {
      return centerCoordinates;
    }
    if (selectedVehicleId) {
      const v = vehicles.find((vh) => vh.id === selectedVehicleId);
      if (v && v.position && !isNaN(v.position[0]) && !isNaN(v.position[1]))
        return v.position;
    }
    if (
      vehicles.length === 1 &&
      vehicles[0].position &&
      !isNaN(vehicles[0].position[0]) &&
      !isNaN(vehicles[0].position[1])
    ) {
    ...
    if (
      ...
      !isNaN(routePolyline[0][0]) &&
      !isNaN(routePolyline[0][1])
    ) {
  ...
  const initialZoom = useMemo(() => {
    ...
    if (
      selectedVehicleId ||
      (vehicles.length === 1 && vehicles[0].position)
    )
      return 16;
    return 5;
  ...
  const getVehicleIcon = (vehicle: ProcessedVehicle) => {
    ...
    if (
      vehicleIdStr.startsWith('start-route-') ||
      vehicleIdStr.startsWith('end-route-')
    ) {
      const type = vehicleIdStr.startsWith('start-route-')
        ? 'start'
        : 'end';
    ...
    >
      ...
      >
        ...
        {shouldAutoFit ? (
          ...
          <ReactiveMapView
            center={centerCoordinates ?? null}
            zoom={zoomLevel || initialZoom}
          />
        )}
        ...
        {routePolyline && routePolyline.length > 1 && (
          <Polyline
            positions={routePolyline.filter(
              (p) =>
                Array.isArray(p) &&
                p.length === 2 &&
                !isNaN(p[0]) &&
        ...
        {displayGeofences.map((geofence) => {
          ...
          if (
            ...
            geofence.definition.center &&
            geofence.definition.radius
          ) {
            const centerLatLng: LatLngExpression = [
              geofence.definition.center[1],
              geofence.definition.center[0],
            ];
            if (
              isNaN(centerLatLng[0]) ||
              isNaN(centerLatLng[1]) ||
              isNaN(geofence.definition.radius)
            )
              return null;
            return (
              <Circle
              ...
              >
                <Popup>
                  ...
                  Tipe: Lingkaran
                  <br />
                  Aturan: {geofence.rule_type || 'N/A'}
                </Popup>
              </Circle>
            );
          } else if (
            geofence.type === 'polygon' &&
            geofence.definition.coordinates &&
            geofence.definition.coordinates[0]
          ) {
            const polygonLatLngs: LatLngExpression[] =
              geofence.definition.coordinates[0]
                .map((coord: number[]) => {
                  if (
                    Array.isArray(coord) &&
                ...
                >
                  ...
                  >
                    {isRouteMarker
                      ? `${
                          ...
                          vehicleIdStr.includes('start-route-')
                            ? 'Titik Mulai'
                            : 'Titik Akhir'
                        })`
                      : vehicle.name}
                  ...
                  {vehicle.timestamp && (
                    ...
                    >
                      <strong>Waktu:</strong>
                      <br />
                      {new Date(vehicle.timestamp).toLocaleString('id-ID', {
                        dateStyle: 'medium',
                        timeStyle: 'short',
                      })}
                    </div>
                  ...
                  {!isRouteMarker && (
                    ...
                    >
                      ...
                      {vehicle.fuel !== null && (
                        <div>BBM: {vehicle.fuel.toFixed(1)}%</div>
                      )}
                      {vehicle.battery !== null && (
                        <div>Baterai: {vehicle.battery.toFixed(1)}V</div>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gray-50 bg-opacity-90 rounded-lg pointer-events-none">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm">
                Lokasi kendaraan, rute, atau geofence akan muncul di sini.
              </p>
            </div>
          </div>
        )}
