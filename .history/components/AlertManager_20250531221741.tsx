...
export function AlertManager({ 
  ...
  const [alerts, setAlerts] = useState<ManagedAlert[]>(() => {
    // Inisialisasi dengan alert awal jika ada
    // Anda mungkin ingin menambahkan alert statis/contoh Anda di sini juga jika perlu
    const staticExampleAlerts: ManagedAlert[] = [
        {
            id: `static-${internalAlertIdCounter++}`, type: "speed_limit", vehicleName: "Van-003", 
            message: "Batas kecepatan terlampaui: 85 km/jam di zona 60 km/jam",
            locationString: "Jl. Gatot Subroto, Jakarta", timestamp: new Date("2024-01-25T14:30:25Z"), 
            status: "active", severity: "high", geofenceName: null
        },
        {
            id: `static-${internalAlertIdCounter++}`, type: "geofence_exit", vehicleName: "Truck-001", 
            message: "Kendaraan keluar dari area resmi: Gudang A",
            locationString: "Dekat Jl. Sudirman, Jakarta", timestamp: new Date("2024-01-25T13:45:12Z"), 
            status: "acknowledged", severity: "medium", geofenceName: "Gudang A"
        },
    ];
    return [...staticExampleAlerts, ...initialAlerts].sort((a,b) => b.timestamp.getTime() - a.timestamp.getTime());
  });
  ...
  useEffect(() => {
    if (newDetectedEvents.length > 0) {
      const transformedAlerts = newDetectedEvents.map((event): ManagedAlert => {
        ...
        let message = `Kendaraan '${event.vehicle_name}' <span class="math-inline">\{event\.event\_type\.replace\('\_', ' '\)\} geofence '</span>{event.geofence_name}'.`;

        if (event.event_type.startsWith('violation_')) {
          severity = "high";
          // type bisa tetap 'violation_enter' atau diubah untuk display
          message = `PELANGGARAN: Kendaraan '${event.vehicle_name}' <span class="math-inline">\{event\.event\_type\.includes\('enter'\) ? 'memasuki' \: 'meninggalkan'\} geofence '</span>{event.geofence_name}' (Aturan: ${event.rule_triggered}).`;
        } else if (event.event_type === 'enter') {
          severity = "low";
          message = `INFO: Kendaraan '<span class="math-inline">\{event\.vehicle\_name\}' memasuki geofence '</span>{event.geofence_name}'.`;
        } else if (event.event_type === 'exit') {
          severity = "low";
          message = `INFO: Kendaraan '<span class="math-inline">\{event\.vehicle\_name\}' meninggalkan geofence '</span>{event.geofence_name}'.`;
        }
