...
export interface Geofence {
  ...
  definition: {
    ...
    center?: number[];
    radius?: number;
    type: string; // Type of shape in definition, e.g., "Circle", "Polygon"
  };
  date_created: string;
}
...
interface User {
  id: string;
  user_id: string; // Can be alias for id
...
export function GeofenceManager() {
  ...
  const [newGeofence, setNewGeofence] = useState({
    name: "",
    description: "", // Although not used in form, good to keep if API supports
    ruleType: "FORBIDDEN", // Default rule type
    type: "polygon" as "polygon" | "circle" // Default shape type
  });
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .leaflet-container {
        z-index: 1 !important; /* Lower than dialogs but higher than page background */
      }
      ...
      .fixed[role="dialog"] { /* ShadCN specific selector for overlay might be needed */
        z-index: 50000 !important;
      }
      
      /* Dialog overlay background */
      [data-state="open"][data-overlay] { /* Example for ShadCN overlay */
  ...
  const validateGeofenceCoordinates = (geofence: Geofence): boolean => {
    try {
      ...
      if (geofence.type === 'circle') {
        // For circles, check center and radius from the definition.
        // The 'coordinates' might be a polygonal representation if stored that way.
      ...
      if (geofence.type === 'polygon') {
        // For polygons, validate the 'coordinates' field.
  ...
  const validatePolygonCoords = (coordinates: number[][][] | undefined): boolean => {
    ...
    // Assuming GeoJSON-like structure: [[[lng, lat], [lng, lat], ...]]
    const coords = coordinates; // Might be coordinates[0] depending on exact structure
    ...
    const firstPoint = coords[0][0]; // First point of the first ring
  ...
  const zoomToGeofence = (geofence: Geofence | null) => {
    ...
    setMapKey(prev => prev + 1); // Re-render map centered on this geofence
  };
  ...
  const getGeofenceCenter = (geofence: Geofence | null): [number, number] => {
    ...
    if (geofence.type === 'circle' && geofence.definition.center && geofence.definition.center.length === 2) {
      ...
      return [geofence.definition.center[1], geofence.definition.center[0]] as [number, number];
    } else if (geofence.type === 'polygon' && geofence.definition.coordinates && geofence.definition.coordinates[0]?.length > 0) {
      const coords = geofence.definition.coordinates[0]; // Assuming first ring
      if (coords.length === 0) return DEFAULT_CENTER;
      // Calculate centroid (simple average for now, consider a more robust method if needed)
  ...
  const prepareGeofenceForMap = (geofence: Geofence) => {
    ...
    try {
      // Ensure the definition structure matches what MapWithDrawing expects
      // MapWithDrawing might expect GeoJSON or a specific Leaflet structure
      return {
        geofence_id: geofence.geofence_id,
        user_id: geofence.user_id,
        name: geofence.name,
        type: geofence.type, // 'circle' or 'polygon'
        rule_type: geofence.rule_type,
        status: geofence.status,
        definition: geofence.definition, // Pass the whole definition
  ...
  const mapGeofences = useMemo(() => {
    return geofences
      ...
      .filter(g => g !== null) as Geofence[]; // Assert type after filtering nulls
  }, [geofences]);
  ...
  useEffect(() => {
    ...
    const loadUser = () => {
      try {
        ...
        if (userJson) {
          ...
          setCurrentUser(user);
          const userId = user.id || user.user_id; // Handle both possible user ID fields
          if (userId) {
    ...
    }, 1000); // Give map a bit of time to initialize, adjust as needed
  ...
  const fetchGeofences = async (userId: string): Promise<Geofence[]> => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return validGeofences; // Return the validated geofences
      } else {
  ...
  const handleAssignGeofence = (geofence: Geofence) => {
    ...
    // Pre-select vehicles already assigned to this geofence
  ...
  const updateVehicleGeofence = async (vehicleId: string, geofenceId: string | null) => {
    try {
      const response = await fetch(`${VEHICLE_API_ENDPOINT}/${vehicleId}`, {
        ...
        body: JSON.stringify({
          geofence_id: geofenceId // Send null to unassign
        })
      });
  ...
  const handleSaveGeofence = async () => {
    ...
    try {
      if (drawnLayers.length === 0 || !L) { // Also check if L is defined
        throw new Error('No drawn layers found or Leaflet not loaded');
      }
      ...
      if (layer instanceof L.Circle) {
        ...
        // The polygonal representation is for systems that might only support polygons.
        const points = 64; 
        const circlePolygonCoords: number[][] = [];
        for (let i = 0; i < points; i++) {
          const angle = (i * 2 * Math.PI) / points;
          // Approximation for converting radius in meters to lat/lng degrees
          // This is a simplification; more accurate conversion might be needed for large radii or near poles.
          ...
          circlePolygonCoords.push([lng, lat]); // GeoJSON format: [lng, lat]
        }
        circlePolygonCoords.push(circlePolygonCoords[0]); // Close the polygon

        definition = {
            type: "Circle", // Descriptive type for the shape
            center: [center.lng, center.lat], // Store center as [lng, lat]
            radius: radius,
            coordinates: [circlePolygonCoords] // Store as GeoJSON-like polygon
        };
      } else if (layer instanceof L.Polygon) {
        // For polygons (including rectangles drawn as polygons)
        const latlngs = (layer as L.Polygon).getLatLngs()[0] as L.LatLng[]; // Assuming simple polygon (no holes)
        const coordinates = latlngs.map(latLng => [latLng.lng, latLng.lat]); // GeoJSON format: [lng, lat]
        ...
        definition = {
            type: "Polygon",
            coordinates: [coordinates] // Store as GeoJSON-like polygon
        };
      } else {
        throw new Error('Invalid layer type for geofence');
      }

      const geofencePayload: Omit<Geofence, 'geofence_id' | 'date_created'> & { date_created?: string } = {
        user_id: userId,
        name: newGeofence.name,
        type: layer instanceof L.Circle ? "circle" : "polygon", // This is the primary type for filtering/display
        rule_type: newGeofence.ruleType as Geofence['rule_type'],
        status: "active", // Default to active
        // date_created: new Date().toISOString(), // Usually set by the server
        definition
      };
      ...
      if (response.ok) {
        ...
        if (userId) {
          ...
          if (newlySavedGeofenceFromList && validateGeofenceCoordinates(newlySavedGeofenceFromList)) {
            ...
            // Fallback: if the saved geofence (from response) is valid, use it.
            // If not, user might need to refresh or check the list.
            if (geofences.length > 0) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (currentGeofence?.geofence_id === geofenceId) {
          ...
          const remainingGeofences = geofences.filter(g => g.geofence_id !== geofenceId);
          if (remainingGeofences.length > 0) { 
            const firstValid = remainingGeofences.find(validateGeofenceCoordinates);
            if (firstValid) zoomToGeofence(firstValid);
            else setMapKey(prev => prev + 1); // Reset map if no valid ones left
          } else {
            setMapKey(prev => prev + 1); // Reset map if no geofences left
          }
        }
      } else {
        const errorData = await response.json();
  ...
  // Fix for Leaflet _leaflet_pos error on geofence deletion or list changes
  useEffect(() => {
    if (!mapReady) return;

    // Force map re-render by updating mapKey after geofence list changes
    const handleGeofenceChange = () => {
      setMapKey(prev => prev + 1);
    };

    // Listen for geofence changes (e.g., after add/delete)
    handleGeofenceChange(); // Call it to ensure map updates if geofences array changes

  }, [geofences, mapReady]); // Rerun if geofences list or map readiness changes
  ...
  return (
    <div className="p-4 sm:p-6 max-w-full mx-auto bg-gray-50 min-h-screen">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 pb-4 border-b">
        ...
        {(!loading || geofences.length > 0 || !currentUser) && !isCreatingGeofence && ( // Show if not loading OR have geofences OR if user not loaded (to allow creation if no geofences yet)
          <Button 
            ...
            disabled={!mapReady || !currentUser} // Disable if map is not ready or no user
          >
      ...
      {isMobile ? (
            ...
            <TabsContent value="list" className="mt-2">
                ...
                {!loading && filteredGeofences.length === 0 && (
                    <Card className="shadow-sm">
                        ...
                        {!searchTerm && (
                            <Button onClick={() => { handleStartCreatingGeofence(); setActiveTab("map"); }} className="bg-green-500 hover:bg-green-600" disabled={!mapReady || !currentUser}>
                ...
                <div className="space-y-3">
                    {filteredGeofences.map((geofence) => (
                        <Card 
                            ...
                            onClick={() => {
                                if (validateGeofenceCoordinates(geofence)) {
                                    zoomToGeofence(geofence); setActiveTab("map");
                                } else {
                                    toast.error(`Geofence "${geofence.name}" memiliki data tidak valid dan tidak dapat ditampilkan.`);
                                }
                            }}
                        >
                        ...
                        <div className={`h-80 sm:h-96 border rounded-lg overflow-hidden shadow-inner ${isCreatingGeofence ? 'ring-2 ring-blue-500' : ''}`}>
                            ...
                            {mapReady && (
                                <MapWithDrawing
                                    ...
                                    geofences={isCreatingGeofence ? [] : (currentGeofence && currentMapGeofence ? [currentMapGeofence] : mapGeofences)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-180px)] xl:h-[calc(100vh-160px)]"> {/* Adjusted height */}
          <div className="lg:col-span-1 flex flex-col bg-white p-4 rounded-xl shadow-lg border border-gray-200">
                ...
                <CardContent className="p-4 space-y-3">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      > <CircleIcon className="h-3.5 w-3.5 mr-1.5" /> Lingkaran </Button>
                    </div>
            ...
            {!loading && filteredGeofences.length === 0 && !isCreatingGeofence && (
              <Card className="flex-1 border-dashed border-gray-300 bg-gray-50 shadow-none">
                <CardContent className="p-6 text-center flex flex-col items-center justify-center h-full">
                  ...
                  <Button onClick={handleStartCreatingGeofence} className="bg-blue-500 hover:bg-blue-600 text-sm" disabled={!mapReady || !currentUser}>
                    <Plus className="h-4 w-4 mr-2" />
            ...
            {filteredGeofences.length > 0 && !isCreatingGeofence && (
            <div className="flex-1 overflow-auto space-y-2 pr-1 custom-scrollbar">
                {filteredGeofences.map((geofence) => (
                  <Card
                    ...
                    onClick={() => {
                        if (validateGeofenceCoordinates(geofence)) {
                            setIsCreatingGeofence(false); // Exit creating mode if was active
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-0"> {/* No gap for continuous sections */}
              ...
              <Card className="shadow-none border-none">
                ...
                <CardContent className="p-0 space-y-3 text-sm">
                  ...
                  {currentGeofence?.type === 'circle' && currentGeofence.definition.radius !== undefined && (
                   ...
                   {currentGeofence?.definition?.coordinates && currentGeofence.definition.coordinates[0] && (
                    <div>
            ...
            <div className="md:col-span-3 p-1 md:p-2 bg-gray-100">
              <div className="h-[300px] md:h-[450px] lg:h-[500px] border rounded-md overflow-hidden shadow-inner">
                  {currentGeofence && mapReady && validateGeofenceCoordinates(currentGeofence) && currentMapGeofence && ( // check valid coords and currentMapGeofence
                        <MapWithDrawing
                            ...
                            viewOnly={true}
                            geofences={[currentMapGeofence]}
                            selectedGeofence={currentMapGeofence}
                            isCreating={false}
                        />
                  )}
                  {(!currentGeofence || !validateGeofenceCoordinates(currentGeofence) || !currentMapGeofence) && mapReady && (
                    ...
                    return (
                        ...
                        >
                            <Checkbox 
                                ...
                                disabled={isAssignedToOtherGeofence || false} // Disable checkbox if assigned to another geofence
                                                ...
                                                if (window.confirm(`Apakah Anda yakin ingin melepaskan assignment kendaraan ${vehicle.name} dari geofence ini?`)) {
                                                    updateVehicleGeofence(vehicle.vehicle_id, null).then(success => {
                                                        if (success) {
                                                            ...
                                                            const userId = currentUser?.id || currentUser?.user_id;
                                                            if (userId) fetchVehicles(userId); // Refresh vehicle list
                                                            setSelectedVehicles(prev => prev.filter(id => id !== vehicle.vehicle_id)); // Uncheck it
                                                        } else {
