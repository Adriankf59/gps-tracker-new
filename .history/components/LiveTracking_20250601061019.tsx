...
export function LiveTracking() {
  ...
  const { 
    ...
    userId ? `${VEHICLE_API_ENDPOINT_BASE}?filter[user_id][_eq]=${userId}` : null,
    fetcher,
    swrConfig
  );
  ...
  const gpsIds = useMemo(() => {
    ...
    return vehicles.map((v: Vehicle) => v.gps_id).filter((id: string | null) => id != null && id.trim() !== '');
  }, [vehicles]);
  ...
  const { 
    ...
    {
      ...swrConfig,
      refreshInterval: 10000, // Data kendaraan refresh lebih cepat
    }
  );

  // SWR untuk geofence detail yang dipilih
  const { 
    ...
    async (url) => {
      ...
      return data; // Karena detail geofence returnnya langsung object, bukan array
    },
    { 
      refreshInterval: 60000, // Geofence detail jarang berubah
  ...
  const processedVehicles = useMemo((): VehicleWithTracking[] => {
    ...
    }).sort((a, b) => (parseInt(a.vehicle_id) || 0) - (parseInt(b.vehicle_id) || 0));
  }, [vehicles, vehicleDataPoints]);

  // Process vehicle untuk map
  ...
  // Process geofence untuk map display
  ...
  const handleVehicleSelect = useCallback(async (vehicle: VehicleWithTracking) => {
    ...
    // Set geofence untuk display dari SWR data
    if (selectedGeofenceDetail && validateGeofenceCoordinates(selectedGeofenceDetail)) {
      setAssignedGeofenceForDisplay(selectedGeofenceDetail);
    } else {
      setAssignedGeofenceForDisplay(null);
    }
  }, [selectedGeofenceDetail, validateGeofenceCoordinates]);

  // Setup geofence detection
  useEffect(() => {
  ...
  // Handle vehicle selection dan auto-select first vehicle
  useEffect(() => {
  ...
  // Loading states
  const isLoading = vehiclesLoading || vehicleDataLoading || geofencesLoading;
  const hasError = vehiclesError || vehicleDataError || geofencesError;
  const isRefreshing = !isLoading && (vehiclesLoading || vehicleDataLoading);

  if (isLoading && processedVehicles.length === 0 && !hasError) {
    return (
