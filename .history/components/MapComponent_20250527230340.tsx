...
const MapComponent: React.FC<MapComponentProps> = ({ 
  ...
  const processedVehicles = useMemo(() => {
    return vehicles.map(vehicle => {
      // Get latest GPS data for this vehicle based on data_id (highest data_id = latest)
      const latestGpsData = vehicleData
        ...
        .sort((a, b) => {
          // Sort by data_id in descending order (latest first)
          const aId = parseInt(a.data_id) || 0;
          const bId = parseInt(b.data_id) || 0;
          return bId - aId;
        })[0];
  ...
  return (
    <div className="relative">
      ...
      {processedVehicles.length > 0 && (
        <div className="absolute top-4 left-4 bg-blue-600 text-white px-3 py-1 rounded-lg text-sm z-[1000]">
          {processedVehicles.length} Vehicle{processedVehicles.length !== 1 ? 's' : ''}
        </div>
      )}
