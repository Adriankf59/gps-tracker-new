...
export function GeofenceManager() {
  ...
  const fetchData = useCallback(async (url: string, options: RequestInit = {}): Promise<any> => {
    try {
  ...
  const fetchGeofences = useCallback(async (userId: string): Promise<Geofence[]> => {
    try {
      const result = await fetchData(`${GEOFENCE_API}?filter[user_id][_eq]=${userId}&limit=-1&sort=-date_created`);
      if (!result) return [];
      
      const parsed: Geofence[] = ensureArray(result.data || result).map((gf: any) => ({
        ...gf,
  ...
  const fetchVehicles = useCallback(async (userId: string): Promise<Vehicle[]> => {
    try {
      const result = await fetchData(`${VEHICLE_API}?filter[user_id][_eq]=${userId}&limit=-1`);
      if (!result) return [];
      
      const fetched: Vehicle[] = ensureArray(result.data || result);
      setVehicles(fetched);
      return fetched;
    } catch (error: unknown) {
  ...
  const handleDrawCreated = (e: DrawEvent): void => {
  ...
  const handleSaveGeofence = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      toast.error(`Failed to save geofence: ${message}`);
    } finally {
      setLoading(false);
    }
  }, [currentUser, newGeofence, drawnLayers, fetchData, refreshData]);

  const handleDeleteGeofence = useCallback(async (geofenceId: string): Promise<void> => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Unknown error';
      toast.error(`Failed to delete geofence: ${message}`);
    } finally {
      setLoading(false);
    }
  }, [vehicles, fetchData, refreshData, currentUser]);

  const handleAssignVehicles = (geofence: Geofence): void => {
  ...
  const saveVehicleAssignments = useCallback(async (): Promise<void> => {
  ...
  const filteredGeofences = useMemo((): Geofence[] => {
  ...
  const displayedGeofences = useMemo((): Geofence[] => {
