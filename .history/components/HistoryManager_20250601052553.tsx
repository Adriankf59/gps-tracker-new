...
export function HistoryManager() {
  ...
  // Fetch vehicle data with improved filtering and pagination
  const fetchVehicleData = async (vehicleId: string, from?: Date, to?: Date) => {
    ...
    try {
      ...
      const token = getAuthToken();
      let url = `http://ec2-13-229-83-7.ap-southeast-1.compute.amazonaws.com:8055/items/vehicle_datas?limit=5000&sort=timestamp`;
      
      // Build filter for vehicle data using only gps_id
      const filters = [];
      ...
      // Only fetch available fields - removed vehicle_id from the list
      filters.push('fields=vehicle_datas_id,gps_id,timestamp,latitude,longitude,speed,fuel_level,battery_level,ignition_status,satellites_used');
      ...
      if (!response.ok) {
        ...
        // If 403, try without field selection
        if (response.status === 403) {
          console.log('Retrying without field selection...');
          const retryUrl = `http://ec2-13-229-83-7.ap-southeast-1.compute.amazonaws.com:8055/items/vehicle_datas?limit=5000&sort=timestamp&filter[gps_id][_eq]=${gpsId}` + 
            (from && to ? `&filter[timestamp][_between]=${from.toISOString()},${to.toISOString()}` : '');
          ...
          if (!retryResponse.ok) {
            throw new Error(`Failed to fetch vehicle data: ${retryResponse.statusText}`);
          }
