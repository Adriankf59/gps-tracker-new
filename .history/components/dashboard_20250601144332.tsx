...
const useAlerts = () => {
  ...
  const processedAlerts = useMemo(() => {
    ...
    return data.map((alert: any) => ({
      ...
      lokasi: alert.lokasi || 'Unknown location' // Fixed incomplete line
    }));
  }, [data]);
...
export function Dashboard() {
  ...
  }, [vehiclesWithData, alerts, geofences.length]); // Added alerts to dependency array for activeAlerts: Math.max
    ...
    return alerts
      ...
      .map((alert) => {
        ...
        return {
          ...
          time: timestamp.toISOString(), // Store ISO string for sorting
          timeDisplay: getRelativeTime(timestamp), // Display time
