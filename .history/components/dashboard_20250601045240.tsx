...
import { getAlerts, Alert } from "@/lib/alertService";
...
export function Dashboard() {
  ...
  // Add state for alerts with proper type
    ...
    const fetchAlerts = async () => {
      try {
        ...
        if (response.data) {
          ...
          // Update activeAlerts count in stats
          setStats(prev => ({
            ...prev,
            activeAlerts: response.data.length
          }));
        }
      } catch (error) {
  ...
  const generateRecentActivity = () => {
    return alerts
      .map((alert) => ({
        id: alert.id || Date.now(), // Use alert ID if available, otherwise timestamp
        vehicle: `Vehicle ${alert.vehicle_id}`,
        event: alert.alert_message,
        time: getRelativeTime(new Date(alert.timestamp)),
        type: alert.alert_type.includes('geofence') ? 'geofence' : 
              alert.alert_type.includes('command') ? 'command' : 'alert'
      }))
