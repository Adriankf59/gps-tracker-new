...
interface WebSocketMessage {
  type: 'vehicles' | 'vehicle_data' | 'geofences' | 'alerts' | 'ping' | 'error';
  data?: any;
  error?: string;
}
...
// WebSocket Hook with API fallback
const useWebSocket = (userId?: string) => {
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch vehicle data
      const vehicleDataResponse = await fetch(`/api/vehicle-data?user_id=${userId}`);
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        // Send subscribe message with Directus format
        ...
        try {
          ...
          switch (message.type) {
            ...
            case 'error':
              ...
              if (message.error !== 'WebSocket is closed before the connection is established.') {
                toast.error(message.error || 'WebSocket error occurred');
              }
              break;
          }
        } catch (error) {
      ...
      ws.onclose = (event) => {
        ...
        // Only reconnect if it was a normal close and we're online
  ...
  useEffect(() => {
    ...
    // Then connect WebSocket after a delay to avoid StrictMode issues
...
export function Dashboard() {
  ...
  useEffect(() => {
    if (!isInitialLoading || vehicles.length > 0) {
      setVehiclesLoading(false);
    }
  }, [vehicles, isInitialLoading]);
  ...
  const handleRefresh = useCallback(async () => {
    ...
    // Also refresh from API as fallback
    await refreshFromAPI();
  }, [isConnected, isOnline, sendMessage, userId, refreshFromAPI]);

  // Loading state
  if (isInitialLoading && processedVehicles.length === 0) {
    return (
