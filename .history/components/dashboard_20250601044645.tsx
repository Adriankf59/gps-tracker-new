    ...
    vehicles.forEach(vehicle => {
      if (vehicle.latestData) {
        ...
        if (fuelLevel > 0 && fuelLevel < 20) {
          const alertMessage = `${vehicle.name}: Low fuel warning (${fuelLevel.toFixed(1)}%)`;
          const alertKey = `fuel_${vehicle.vehicle_id}_${fuelLevel.toFixed(1)}`;
          ...
          // Show toast alert only if it's not a duplicate
          if (!isInitialLoad.current && !previousAlerts.current.has(alertKey)) {
            toast.error(alertMessage, {
              description: `Current fuel level: ${fuelLevel.toFixed(1)}%`,
              duration: 5000
            });
          }
        ...
        // Speed alert
        if (speed > 80) {
          const alertMessage = `${vehicle.name}: Speed limit exceeded (${speed} km/h)`;
          const alertKey = `speed_${vehicle.vehicle_id}_${speed}`;
          ...
          if (!isInitialLoad.current && !previousAlerts.current.has(alertKey)) {
            toast.error(alertMessage, {
              description: `Current speed: ${speed} km/h`,
              duration: 5000
