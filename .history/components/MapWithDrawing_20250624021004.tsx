...
export function GeofenceManager() {
  ...
  const handleDrawCreated = (e: DrawCreatedEvent) => {
    ...
    // Handle circle that was converted to polygon
    if (e.originalType === 'circle') {
      setNewGeofence(prev => ({ ...prev, type: 'circle' }));
      toast.success('Circle drawn successfully');
    } else {
  ...
  const handleSaveGeofence = useCallback(async () => {
    ...
    try {
      ...
      // All shapes are now polygons (circles are converted in MapWithDrawing)
      ...
      const payload = {
        ...
        type: newGeofence.type,  // Keep the original type for display
  ...
  const handleDeleteGeofence = useCallback(async (geofenceId: number) => {
    ...
    try {
      ...
      // Clear current geofence if it was deleted
      ...
      if (userId) {
        ...
        // Set the first geofence as current if available
        setGeofences(prevGeofences => {
          const remaining = prevGeofences.filter(g => g.geofence_id !== geofenceId);
          if (remaining.length > 0 && !currentGeofence) {
            setCurrentGeofence(remaining[0]);
          }
          return remaining;
        });
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" style={{ minHeight: 'calc(100vh - 200px)' }}>
        ...
        <div className="lg:col-span-2 border border-slate-200 rounded-2xl overflow-hidden shadow-2xl bg-white/90 min-h-[300px]">
          <MapWithDrawing
            ...
            zoom={isCreating ? 12 : (currentGeofence ? 13 : 5)}
