...
const MapWithDrawing = React.memo(({
  center,
  zoom = 5,
  drawMode = "polygon",
  onDrawCreated,
    ...
    const loadMapComponents = async () => {
      try {
        ...
        function GeofenceDisplay({ geofenceItem, isSelected }: GeofenceDisplayProps) {
          console.log('🎨 Rendering GeofenceDisplay:', {
            id: geofenceItem.geofence_id,
  ...
  return (
    <div ref={mapContainerRef} className="h-full w-full rounded-md overflow-hidden relative" style={{ minHeight: '400px' }}>
      ...
      >
        ...
        {/* Geofences Display */}
        <FeatureGroup ref={displayLayersRef} key={`display-${geofences.length}-${selectedGeofence?.geofence_id}`}>
