...
export function CommandCenter() {
  ...
  const fetchCommandHistory = async (userId: string) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/items/commands?filter[issued_by][_eq]=${userId}&sort=-date_created&limit=10`
      );
  ...
  const handleSendCommand = async () => {
    ...
    try {
      ...
      toast.success(`Command sent to ${selectedVehicleObj?.name || 'vehicle'}`);
