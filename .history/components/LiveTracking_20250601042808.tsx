...
export function LiveTracking() {
  ...
  // Updated: Improved geofence processing for map display
  const processedGeofenceForMapDisplay = useMemo((): ProjectGeofence[] => {
    const geofencesToDisplay: ProjectGeofence[] = [];
    
    // Always include assigned geofence if available and valid
    if (assignedGeofenceForDisplay && validateGeofenceCoordinates(assignedGeofenceForDisplay)) {
      ...
      geofencesToDisplay.push(assignedGeofenceForDisplay);
    }
    
    return geofencesToDisplay;
  ...
  const fetchAllVehicleData = useCallback(async (vehicleList: Vehicle[]): Promise<VehicleData[]> => {
    ...
    try {
      ...
      const url = `${VEHICLE_DATA_API_ENDPOINT_BASE}?filter[gps_id][_in]=${gpsIds.join(',')}&limit=${gpsIds.length * 10}&sort=-timestamp`;
  ...
  // Updated: Enhanced geofence fetching with better error handling
  const fetchGeofenceDetailsForDisplay = useCallback(async (geofenceId: string | number | null): Promise<ProjectGeofence | null> => {
    if (!geofenceId) {
      console.log('Tidak ada geofence_id untuk diambil');
      return null;
    }
    
    // First, check in cached geofences
    const cachedGeofence = allUserGeofences.find(gf => gf.geofence_id.toString() === geofenceId.toString());
    if (cachedGeofence) {
      console.log(`Menggunakan geofence dari cache: ${cachedGeofence.name}`);
    ...
    try {
      ...
      if (result.data && validateGeofenceCoordinates(result.data as ProjectGeofence)) {
        console.log(`Berhasil fetch geofence individual: ${result.data.name}`);
        return result.data as ProjectGeofence;
      } else { 
        console.warn("Data geofence tidak valid setelah fetch individual:", result.data); 
        return null; 
      }
    } catch (error) { 
      console.error('Error mengambil detail geofence individual:', error); 
      return null; 
    }
  ...
  const mergeAndProcessData = useCallback((fetchedVehicles: Vehicle[], allVehicleData: VehicleData[]): VehicleWithTracking[] => {
    return fetchedVehicles.map(vehicle => {
      const latestData = allVehicleData.find(data => data.gps_id && vehicle.gps_id && data.gps_id === vehicle.gps_id);
      return { 
        ...vehicle, 
        latestData, 
  ...
  // Updated: Enhanced vehicle selection handler with geofence loading
  const handleVehicleSelect = useCallback(async (vehicle: VehicleWithTracking) => {
    ...
    // Load assigned geofence for display
    if (vehicle.geofence_id) {
      console.log(`Loading geofence ${vehicle.geofence_id} untuk kendaraan ${vehicle.name}`);
      try {
        const geofenceData = await fetchGeofenceDetailsForDisplay(vehicle.geofence_id);
        if (geofenceData) {
          console.log(`Geofence berhasil dimuat untuk display: ${geofenceData.name}`);
          setAssignedGeofenceForDisplay(geofenceData);
        } else {
          console.warn(`Gagal memuat geofence ${vehicle.geofence_id} untuk display`);
          setAssignedGeofenceForDisplay(null);
        }
      } catch (error) {
        console.error('Error loading geofence untuk display:', error);
        setAssignedGeofenceForDisplay(null);
      }
    } else {
      console.log('Kendaraan tidak memiliki geofence ter-assign');
      setAssignedGeofenceForDisplay(null);
    }
  ...
  const loadTrackingData = useCallback(async (isRefresh = false) => {
    if (!userId) {
      if (!isRefresh) setError("User ID tidak ditemukan. Silakan login kembali.");
      setLoading(false); 
      setRefreshing(false);
      return;
    }
    ...
    try {
        ...
        if (vehicle.latestData?.latitude && vehicle.latestData?.longitude && vehicle.latestData.timestamp) {
          ...
          if (detectionResult.events.length > 0) {
            for (const event of detectionResult.events) {
              ...
              // Save violation events to alerts endpoint
      ...
      // Handle vehicle selection and geofence display
      let vehicleToDisplay = sortedVehicles.find(v => v.vehicle_id === selectedVehicleId);
      if (!vehicleToDisplay && sortedVehicles.length > 0) {
        vehicleToDisplay = sortedVehicles[0];
      }
      
      if (vehicleToDisplay) {
        // Use the enhanced vehicle selector
        await handleVehicleSelect(vehicleToDisplay);
      } else {
        setSelectedVehicleId(null); 
        setSelectedVehicleName(null); 
        setSelectedVehicleCoords(null); 
        ...
        <div className="flex items-center gap-3">
            ...
            <div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {/* Display geofence info for selected vehicle */}
        ...
        <div className="md:col-span-2 lg:col-span-3">
          <Card className="overflow-hidden shadow-lg border rounded-xl">
            <CardContent className="p-0">
              <div className="rounded-b-xl overflow-hidden m-4" style={{height: 'calc(100vh - 370px)', minHeight: '450px'}}>
                <MapComponent 
              ...
              {vehicles.length > 0 ? (
                vehicles.map((vehicle) => (
                  ...
                  >
                    ...
                    {/* Show assigned geofence name if exists */}
