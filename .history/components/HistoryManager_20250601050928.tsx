...
interface DirectusFilter {
  _and: Array<{
    vehicle_id?: { _eq: string };
    timestamp?: { _between: [string, string] };
  }>;
}
...
export function HistoryManager() {
  ...
  const fetchVehicleData = async (vehicleId: string, from?: Date, to?: Date) => {
    try {
      const baseUrl = new URL('http://ec2-13-229-83-7.ap-southeast-1.compute.amazonaws.com:8055/items/vehicle_datas');
      ...
      if (from && to) {
        filter._and.push({
          timestamp: {
            _between: [from.toISOString(), to.toISOString()]
          }
        });
      }
      ...
      setVehicleData([]); // Reset data on error
    }
  };
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="h-[500px]">
              <MapComponent
                vehicles={routeData.map((point, index) => ({
                  id: index.toString(),
                  name: `Point ${index + 1}`,
