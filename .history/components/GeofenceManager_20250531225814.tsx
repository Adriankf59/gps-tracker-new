...
export function GeofenceManager() {
  ...
  const handleViewGeofence = (geofence: Geofence) => {
    console.log('🔍 Viewing geofence:', geofence.name);

    if (!geofence) {
      console.error('❌ Invalid geofence: null or undefined');
      toast.error("Data geofence tidak valid");
      return;
    }
    
    if (!validateGeofenceCoordinates(geofence)) {
      console.error('❌ Invalid geofence coordinates for viewing:', geofence);
      toast.error("Koordinat geofence tidak valid. Tidak dapat menampilkan detail.");
      // Do not open dialog if coordinates are invalid
      return;
    }
    
    setCurrentGeofence(geofence); // Set it first
    setIsViewDialogOpen(true); // Then open dialog
    setIsCreatingGeofence(false); // Ensure not in creating mode
    
    // No need to call zoomToGeofence here as the dialog map will handle its own view
    // but ensure mapKey is updated if dialog map relies on it for re-rendering with new geofence.
    // The dialog map's key is `view-dialog-${currentGeofence.geofence_id}` which should re-render.
  };
  ...
  const handleSaveGeofence = async () => {
    ...
    try {
      ...
      let geofencePayload: Omit<Geofence, 'geofence_id'>; // Use Omit for payload
      const currentUserId = currentUser.id || currentUser.user_id;
      const currentDate = new Date().toISOString();

      let definitionPart: Geofence['definition'];

      if (newGeofence.type === "polygon") {
        const latlngs = layer.getLatLngs()[0]; // For simple polygon
        ...
        definitionPart = {
            type: "Circle", // Store the conceptual type as "Circle"
            center: [center.lng, center.lat],
            radius: radius,
            coordinates: [circlePolygonCoords] // Also provide polygon coordinates
        };
      }
      ...
      geofencePayload = {
        ...
        type: newGeofence.type as "circle" | "polygon", // Actual type drawn/selected
                        ...
                        <CardContent className="p-4">
                            ...
                            <div className="flex gap-2 mt-2">
                                <Button
                                    ...
                                    onClick={(e) => { e.stopPropagation(); handleViewGeofence(geofence);}}
                                >
                    ...
                    <CardContent className="p-3">
                      ...
                      <div className="flex gap-1.5 items-center justify-end">
                        <Button
                          ...
                          size="icon"
                          onClick={(e) => { e.stopPropagation(); handleViewGeofence(geofence); }}
                          className="text-sky-600 hover:text-sky-700 hover:bg-sky-50 border-gray-300 h-7 w-7"
                          title="Lihat Detail"
          ...
          <div className={`lg:col-span-2 h-full min-h-[400px] border rounded-xl overflow-hidden shadow-xl ${isCreatingGeofence ? 'ring-2 ring-offset-2 ring-blue-500' : 'border-gray-200'}`}>
            ...
            {mapReady && (
              <MapWithDrawing
                ...
                isCreating={isCreatingGeofence}
                drawnLayersForEditing={isCreatingGeofence ? drawnLayers : undefined} // Pass drawn layers for editing when creating
              />
            )}
          </div>
        </div>
